<script lang="ts">
  ...
  onMount(() => {
    ...
    const handlePointerDown = (event: MouseEvent) => {
      if (
        ansiPresetDropdownOpen &&
        ansiPresetDropdownEl &&
        !ansiPresetDropdownEl.contains(event.target as Node)
      ) {
        ansiPresetDropdownOpen = false
      }
      if (
        terminalThemeDropdownOpen &&
        terminalThemeDropdownEl &&
        !terminalThemeDropdownEl.contains(event.target as Node)
      ) {
  ...
  function updateTerminalScrollback(raw: string) {
    ...
    const clamped = Math.max(
      ...
      Math.min(MAX_TERMINAL_SCROLLBACK, Math.trunc(v))
    )
  ...
  function handleShortcutInput(key: ShortcutKey, raw: string) {
    ...
    if (conflict) {
      shortcutErrors = {
        ...shortcutErrors,
        [key]: `与「${shortcutLabels[conflict.conflictWith]}」冲突`,
      }
      return
    }
  ...
  async function handleRemoveLock() {
    ...
    const confirmed = await confirm('确定要清除应用锁吗？清除后应用启动将不再需要密码。', {
      title: '清除应用锁',
      kind: 'warning',
    })
    if (!confirmed) return
  ...
  // Color key labels
  type CustomThemeKey = Exclude<
    keyof NonNullable<AppSettings['appearance']['customTheme']>,
    'extendedAnsi'
  >
  ...
  const basicColorKeys: CustomThemeKey[] = [
    'background',
    'foreground',
    'cursor',
    'selectionBackground',
  ]
  const standardColorKeys: CustomThemeKey[] = [
    'black',
    'red',
    'green',
    'yellow',
  ...
  const brightColorKeys: CustomThemeKey[] = [
    'brightBlack',
    'brightRed',
    'brightGreen',
    'brightYellow',
  ...
  $: selectedTerminalTheme = terminalThemes.find(
    (theme) => theme.id === $settings.appearance.terminalTheme
  )
...
>
  <div
    class="bg-app-surface border border-app-border rounded-xl shadow-2xl w-full max-w-3xl h-[600px] flex overflow-hidden text-app-text"
  >
    <!-- Sidebar -->
    <div class="w-48 border-r border-app-border bg-app-surface-light p-4 flex flex-col gap-1">
      ...
      {#each tabs as tab}
        <button
          class="text-left px-3 py-2 rounded-lg text-sm font-medium transition-colors {activeTab ===
          tab.id
      ...
      <div class="flex-1 overflow-y-auto p-6 custom-scrollbar">
        ...
        {:else if activeTab === 'shortcuts'}
          <div class="space-y-6" in:slide={{ duration: 200 }}>
            ...
            <div class="space-y-4">
              ...
              <div class="grid grid-cols-2 gap-4 items-center border-b border-app-border pb-4">
                <div>
                  <span class="block text-sm font-medium text-app-text-secondary"
                    >切换文件浏览器面板</span
                  >
                ...
                <div class="space-y-1">
                  ...
                  {#if shortcutErrors.toggleFileBrowser}
                    <div class="text-xs text-red-500 dark:text-red-400">
                      {shortcutErrors.toggleFileBrowser}
                    </div>
                  {/if}
                </div>
              </div>
              ...
              <div class="grid grid-cols-2 gap-4 items-center border-b border-app-border pb-4">
                ...
                <div class="space-y-1">
                  ...
                  {#if shortcutErrors.fileBrowserRefresh}
                    <div class="text-xs text-red-500 dark:text-red-400">
                      {shortcutErrors.fileBrowserRefresh}
                    </div>
                  {/if}
                </div>
              </div>
              ...
              <div class="grid grid-cols-2 gap-4 items-center border-b border-app-border pb-4">
                ...
                <div class="space-y-1">
                  ...
                  {#if shortcutErrors.fileBrowserNewFolder}
                    <div class="text-xs text-red-500 dark:text-red-400">
                      {shortcutErrors.fileBrowserNewFolder}
                    </div>
                  {/if}
                </div>
              </div>
              ...
              <div class="grid grid-cols-2 gap-4 items-center border-b border-app-border pb-4">
                ...
                <div class="space-y-1">
                  ...
                  {#if shortcutErrors.fileBrowserNewFile}
                    <div class="text-xs text-red-500 dark:text-red-400">
                      {shortcutErrors.fileBrowserNewFile}
                    </div>
                  {/if}
                </div>
              </div>
              ...
              <div class="grid grid-cols-2 gap-4 items-center border-b border-app-border pb-4">
                ...
                <div class="space-y-1">
                  ...
                  {#if shortcutErrors.fileBrowserRename}
                    <div class="text-xs text-red-500 dark:text-red-400">
                      {shortcutErrors.fileBrowserRename}
                    </div>
                  {/if}
                </div>
              </div>
              ...
              <div class="grid grid-cols-2 gap-4 items-center border-b border-app-border pb-4">
                ...
                <div class="space-y-1">
                  ...
                  {#if shortcutErrors.fileBrowserDelete}
                    <div class="text-xs text-red-500 dark:text-red-400">
                      {shortcutErrors.fileBrowserDelete}
                    </div>
                  {/if}
                </div>
              </div>
              ...
              <div class="grid grid-cols-2 gap-4 items-center border-b border-app-border pb-4">
                ...
                <div class="space-y-1">
                  ...
                  {#if shortcutErrors.fileBrowserSelectAll}
                    <div class="text-xs text-red-500 dark:text-red-400">
                      {shortcutErrors.fileBrowserSelectAll}
                    </div>
                  {/if}
                </div>
              </div>
              ...
              <div class="grid grid-cols-2 gap-4 items-center border-b border-app-border pb-4">
                ...
                <div class="space-y-1">
                  ...
                  {#if shortcutErrors.fileBrowserOpen}
                    <div class="text-xs text-red-500 dark:text-red-400">
                      {shortcutErrors.fileBrowserOpen}
                    </div>
                  {/if}
                </div>
              </div>
              ...
              <div class="grid grid-cols-2 gap-4 items-center border-b border-app-border pb-4">
                ...
                <div class="space-y-1">
                  ...
                  {#if shortcutErrors.fileBrowserBack}
                    <div class="text-xs text-red-500 dark:text-red-400">
                      {shortcutErrors.fileBrowserBack}
                    </div>
                  {/if}
                </div>
              </div>
            ...
            <div
              class="bg-app-surface/80 border border-app-border rounded-lg p-3 text-xs text-app-text-secondary backdrop-blur-sm"
            >
              <p>
                提示：点击输入框后直接按下组合键即可录制，按 `Esc` 可清空。支持
                Ctrl、Shift、Alt、Meta (Cmd) 和 F 键、Enter、Delete 等特殊键。
              </p>
            </div>
          </div>
        ...
        {:else if activeTab === 'appearance'}
          <div class="space-y-6" in:slide={{ duration: 200 }}>
            {#if $settings.theme === 'custom'}
              <div
                class="p-4 border border-slate-200 dark:border-slate-700 rounded-xl space-y-3"
                    ...
                    <div>
                      ...
                      <div class="flex gap-2">
                        <input
                          id={inputId}
                          type="color"
                          value={$settings.appearance.customUITheme?.[key] ||
                            defaultCustomUITheme[key]}
            ...
            <div>
              ...
              <div class="relative" bind:this={ansiPresetDropdownEl}>
                ...
                >
                  {#if selectedAnsiPreset}
                    <div class="flex items-center gap-3 min-w-0 flex-1">
                      {#if selectedAnsiPreset.id === 'custom'}
                        <div
                          class="w-20 h-6 rounded border border-slate-300 dark:border-slate-600 bg-slate-100 dark:bg-slate-800 shrink-0"
                        ></div>
                      {:else}
                        <div
                  ...
                  {/if}
                  <svg
                    class="w-4 h-4 text-app-text-secondary shrink-0 transition-transform {ansiPresetDropdownOpen
                      ? 'rotate-180'
                      : ''}"
                    viewBox="0 0 20 20"
                    fill="none"
                  ...
                  >
                    <div class="max-h-80 overflow-y-auto custom-scrollbar py-1">
                      {#each ansiColorPresets as preset}
                        <button
                          type="button"
                          class="w-full px-3 py-2 flex items-center justify-between gap-3 text-left transition-colors {$settings
                            .appearance.ansiColorPreset === preset.id
                        ...
                        >
                          <div class="flex items-center gap-3 min-w-0 flex-1">
                            ...
                            {:else}
                              <div
                                class="flex gap-0.5 p-1 rounded border border-slate-300 dark:border-slate-600 shrink-0"
                                style="background-color: {isLightAnsiPreset(preset.id)
                                  ? '#f8fafc'
                                  : '#1a1a1a'}"
                              >
              ...
              {#if $settings.appearance.ansiColorPreset === 'custom'}
                <div
                  class="mt-4 p-4 border border-slate-200 dark:border-slate-700 rounded-lg bg-slate-50 dark:bg-slate-900/50"
                  transition:slide={{ duration: 200 }}
                >
                  ...
                  <div class="grid grid-cols-2 sm:grid-cols-3 gap-3">
                    {#each ['foreground', 'red', 'green', 'yellow', 'blue', 'magenta'] as key}
                      ...
                      {@const value =
                        $settings.appearance.customAnsiColors?.[key as AnsiColorKey] ||
                        defaultCustomAnsiColors[key as AnsiColorKey]}
                ...
                >
                  {#if selectedTerminalTheme}
                    <div class="flex items-center gap-3 min-w-0 flex-1">
                      ...
                      >
                        <div
                          class="px-1 py-0.5 text-[10px] font-mono truncate"
                      ...
                      <div class="min-w-0 flex-1">
                        <div class="text-sm font-medium text-app-text truncate">
                          {selectedTerminalTheme.name}
                        </div>
                      </div>
                    </div>
                  {/if}
                  <svg
                    class="w-4 h-4 text-app-text-secondary shrink-0 transition-transform {terminalThemeDropdownOpen
                      ? 'rotate-180'
                      : ''}"
                    viewBox="0 0 20 20"
                    fill="none"
                    ...
                    <div class="max-h-80 overflow-y-auto custom-scrollbar py-1">
                      {#each terminalThemes as theme}
                        <button
                          type="button"
                          class="w-full px-3 py-2 flex items-center justify-between gap-3 text-left transition-colors {$settings
                            .appearance.terminalTheme === theme.id
                            ? 'bg-primary-50 dark:bg-primary-500/10'
                            : 'hover:bg-app-bg'}"
                          role="option"
            ...
            {#if $settings.appearance.terminalTheme === 'custom'}
              ...
              >
                ...
                <div class="grid grid-cols-2 gap-4">
                  <!-- Basic Colors -->
                  <div class="space-y-2">
                    <h4 class="text-xs font-medium text-slate-600 dark:text-slate-400 uppercase">
                      基础颜色
                    </h4>
                    {#each basicColorKeys as key (key)}
                      ...
                      <div class="flex items-center gap-2">
                        <label
                          for={inputId}
                          class="text-xs text-slate-600 dark:text-slate-400 w-20 shrink-0"
                          >{label}</label
                        >
                        <input
                          id={inputId}
                          ...
                          {value}
                          on:input={(e) =>
                            updateCustomColor(key, (e.target as HTMLInputElement).value)}
                          class="w-8 h-8 rounded cursor-pointer border border-slate-300 dark:border-slate-600"
                        />
                        <input
                          type="text"
            ...
            <div>
              ...
              >
              <div
                class="border border-slate-200 dark:border-slate-700 rounded-lg p-4 bg-slate-50 dark:bg-slate-900/50 space-y-4"
              >
                <div class="flex items-start gap-4">
                  <!-- Preview -->
                  <div
                    class="relative w-32 h-20 rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-100 dark:bg-slate-800 overflow-hidden shrink-0 group"
                  >
                  ...
                  <div class="flex-1 space-y-3">
                    <div>
                      ...
                      <label
                        class="inline-flex items-center gap-2 px-3 py-1.5 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded text-xs font-medium cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors text-slate-700 dark:text-slate-300"
                      >
                ...
                <div class="pt-4 border-t border-slate-200 dark:border-slate-700 space-y-4">
                  ...
                  <div>
                    <div class="flex justify-between mb-1">
                      <label for="bg-opacity-slider" class="text-xs text-slate-500"
                        >背景不透明度</label
                      >
                      <span class="text-xs text-slate-500"
                        >{Math.round(
                          ($settings.appearance.backgroundOpacity ??
                            ($settings.appearance.backgroundImage ? 0.5 : 1)) * 100
                        )}%</span
                      >
                    </div>
                    <input
                      ...
                      value={$settings.appearance.backgroundOpacity ??
                        ($settings.appearance.backgroundImage ? 0.5 : 1)}
                      on:input={(e) =>
                        updateAppearanceSetting('backgroundOpacity', Number(e.currentTarget.value))}
                    ...
                    <div class="grid grid-cols-2 gap-4">
                      ...
                      <div>
                        <div class="flex justify-between mb-1">
                          ...
                          <span class="text-xs text-slate-500"
                            >{$settings.appearance.backgroundBlur ?? 0}px</span
                          >
                        </div>
                        <input
                          id="bg-blur-slider"
        ...
        {:else if activeTab === 'security'}
          <div class="space-y-6" in:slide={{ duration: 200 }}>
            ...
            {#if securityMessage}
              <div
                class="p-3 bg-green-50 dark:bg-green-500/10 border border-green-200 dark:border-green-500/20 text-green-600 dark:text-green-400 rounded-lg text-sm"
              >
                {securityMessage}
              </div>
            {/if}
            ...
            {#if securityError}
              <div
                class="p-3 bg-red-50 dark:bg-red-500/10 border border-red-200 dark:border-red-500/20 text-red-600 dark:text-red-400 rounded-lg text-sm"
              >
                {securityError}
              </div>
            {/if}
                ...
                <div class="flex items-center justify-between border-t border-app-border pt-4">
                  ...
                  <label class="relative inline-flex items-center cursor-pointer">
                    <input
                      ...
                      on:change={(e) =>
                        updateSecuritySetting('lockOnBlur', (e.target as HTMLInputElement).checked)}
                      class="sr-only peer"
                    />
                    <div
                      class="w-11 h-6 bg-app-border peer-focus:outline-none peer-focus:ring-2 peer-focus:ring-primary-500 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-primary-600"
                ...
                <div class="pt-4 border-t border-app-border">
                  ...
                  <div class="space-y-3">
                    ...
                    <div>
                      <label
                        class="block text-sm font-medium text-app-text-secondary mb-1"
                        for="confirm-pwd-change">确认新密码</label
                      >
                      <input
                        type="password"
                        id="confirm-pwd-change"
                        bind:value={confirmPassword}
                        class="w-full bg-app-bg border border-app-border rounded-lg px-3 py-2 text-app-text focus:border-primary-500 outline-none"
                      />
                    </div>
                    <div class="flex gap-3 pt-2">
                      <button
                        class="px-4 py-2 bg-app-surface-light hover:bg-app-border text-app-text rounded-lg text-sm font-medium transition-colors"
                        on:click={handleChangeLock}
                      >
                        更新密码
                      </button>
                      <button
                        class="px-4 py-2 bg-red-50 dark:bg-red-900/30 hover:bg-red-100 dark:hover:bg-red-900/50 text-red-600 dark:text-red-400 border border-red-200 dark:border-red-900/50 rounded-lg text-sm font-medium transition-colors"
                        on:click={handleRemoveLock}
                      >
                        清除应用锁
                      </button>
                    </div>
          ...
          <div class="space-y-6" in:slide={{ duration: 200 }}>
            ...
            <div class="space-y-4 border border-app-border rounded-lg p-4 bg-app-surface">
              ...
              <div>
                <label
                  class="block text-sm font-medium text-app-text-secondary mb-1"
                  for="ai-base-url">API Base URL</label
                >
                <input
                  type="text"
                  id="ai-base-url"
                ...
                <p class="mt-1 text-xs text-app-text-secondary">
                  这里要填 OpenAI 兼容接口根地址，不是服务官网首页，通常以 `/v1` 结尾。
                </p>
              </div>
              <div>
                <label
                  class="block text-sm font-medium text-app-text-secondary mb-1"
                  for="ai-api-key">API Key</label
                >
                <input
                  type="password"
                  id="ai-api-key"
            ...
            <div class="space-y-4 border border-app-border rounded-lg p-4 bg-app-surface">
              ...
              <div class="grid grid-cols-2 gap-4">
                <div>
                  <label
                    ...
                    for="ai-temp"
                  >
                    温度 <span class="text-xs text-app-text-secondary"
                ...
                <div>
                  <label
                    class="block text-sm font-medium text-app-text-secondary mb-1"
                    for="ai-max-tokens">最大 Token 数</label
                  >
                  <input
                    type="number"
                    id="ai-max-tokens"
              ...
              <div>
                <label
                  class="block text-sm font-medium text-app-text-secondary mb-1"
                  for="ai-context-lines"
                >
                  终端上下文行数 <span class="text-xs text-app-text-secondary"
                    >(附加给 AI 的终端历史行数)</span
                  >
                </label>
                <input
