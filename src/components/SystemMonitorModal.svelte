...
<div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4" role="button" tabindex="0" on:click|self={onClose} on:keydown={(e) => e.key === 'Escape' && onClose()}>
  <div class="bg-slate-900 border border-slate-800 rounded-xl shadow-2xl w-full max-w-4xl max-h-[90vh] flex flex-col overflow-hidden" transition:fade={{ duration: 200 }}>
    <!-- Header -->
    <div class="flex items-center justify-between px-6 py-4 border-b border-slate-800 bg-slate-900">
      <div class="flex items-center gap-3">
        ...
        <h2 class="text-lg font-semibold text-slate-100">系统监控 - {connection.name}</h2>
        {#if lastUpdated}
    ...
    <!-- Content -->
    <div class="flex-1 overflow-y-auto p-6 custom-scrollbar bg-slate-950/50">
        ...
        {:else if error && !lastUpdated}
            <div class="flex flex-col items-center justify-center h-64 text-red-400">
                ...
                <p>{error}</p>
                <button class="mt-4 px-4 py-2 bg-slate-800 rounded hover:bg-slate-700 text-slate-300" on:click={fetchData}>重试</button>
            </div>
        {:else}
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
                <!-- CPU Card -->
                <div class="bg-slate-900 border border-slate-800 rounded-xl p-4 flex flex-col relative overflow-hidden group hover:border-blue-500/50 transition-colors">
                    <h3 class="text-xs font-medium text-slate-400 uppercase tracking-wider mb-2">CPU 使用率</h3>
                    <div class="flex items-end gap-2 mt-auto z-10">
                        <span class="text-3xl font-bold text-white">{stats.cpuUsage}%</span>
                    </div>
                    <!-- Progress Bar -->
                ...
                <!-- Memory Card -->
                <div class="bg-slate-900 border border-slate-800 rounded-xl p-4 flex flex-col relative overflow-hidden group hover:border-purple-500/50 transition-colors">
                    <h3 class="text-xs font-medium text-slate-400 uppercase tracking-wider mb-2">内存使用</h3>
                    <div class="flex items-end gap-2 mt-auto z-10">
                        <span class="text-3xl font-bold text-white">{memPercent}%</span>
                ...
                <!-- Load Average -->
                <div class="bg-slate-900 border border-slate-800 rounded-xl p-4 flex flex-col hover:border-yellow-500/50 transition-colors">
                    <h3 class="text-xs font-medium text-slate-400 uppercase tracking-wider mb-2">平均负载</h3>
                    <div class="flex flex-col gap-1 mt-auto">
                        <div class="flex justify-between text-sm">
                            <span class="text-slate-500">1 min</span>
                            <span class="text-white font-mono">{stats.loadAvg[0] || '-'}</span>
                        </div>
                        <div class="flex justify-between text-sm">
                            <span class="text-slate-500">5 min</span>
                            <span class="text-white font-mono">{stats.loadAvg[1] || '-'}</span>
                        </div>
                        <div class="flex justify-between text-sm">
                            <span class="text-slate-500">15 min</span>
                            <span class="text-white font-mono">{stats.loadAvg[2] || '-'}</span>
                        </div>
                    </div>
                </div>

                <!-- Uptime -->
                <div class="bg-slate-900 border border-slate-800 rounded-xl p-4 flex flex-col hover:border-green-500/50 transition-colors">
                    <h3 class="text-xs font-medium text-slate-400 uppercase tracking-wider mb-2">运行时间</h3>
                    <div class="flex items-center h-full">
                        <span class="text-xl font-medium text-white">{stats.uptime || '-'}</span>
                    </div>
                </div>
            </div>
                        ...
                        <tbody class="divide-y divide-slate-800/50">
                            {#each stats.diskUsage as disk}
                                <tr class="group hover:bg-slate-800/30 transition-colors">
                                    <td class="py-3 font-mono text-slate-300">{disk.mount}</td>
                                    <td class="py-3 text-slate-400">{disk.size}</td>
                                    <td class="py-3 text-slate-400">{disk.used}</td>
                                    <td class="py-3 text-slate-400">{disk.avail}</td>
                                    <td class="py-3">
                                        <div class="flex items-center gap-2">
                                            ...
                                            <span class="text-xs text-slate-400 w-8 text-right">{disk.usePercent}</span>
                                        </div>
                                    </td>
                                </tr>
                            {/each}
