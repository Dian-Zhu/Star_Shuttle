...
<div class="flex flex-col h-full w-full bg-slate-950">
  ...
  {#if $activeTerminals.length > 0}
    <div class="flex bg-slate-900 border-b border-slate-800 overflow-x-auto no-scrollbar">
      {#each $activeTerminals as terminal, index}
        <button
          class="group flex items-center gap-2 px-4 py-2.5 min-w-[160px] max-w-[240px] text-sm border-r border-slate-800 transition-colors relative
          {index === $selectedTerminalIndex 
            ? 'bg-slate-950 text-blue-400 font-medium' 
        ...
        >
          ...
          <span
            role="button"
            tabindex="0"
            class="opacity-0 group-hover:opacity-100 p-1 rounded hover:bg-slate-700 text-slate-400 hover:text-red-400 transition-all"
  ...
  <div class="flex-1 relative overflow-hidden">
    {#if $activeTerminals.length === 0}
      <div class="absolute inset-0 flex flex-col items-center justify-center text-slate-500 bg-slate-950/50">
        <div class="w-16 h-16 mb-4 rounded-2xl bg-slate-900 flex items-center justify-center border border-slate-800">
          <TerminalIcon class="w-8 h-8 opacity-50" />
        </div>
        <p class="text-lg font-medium text-slate-400">无活动会话</p>
