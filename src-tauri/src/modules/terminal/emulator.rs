...
impl super::TerminalEmulator for TerminalEmulatorImpl {
    ...
    fn write(&mut self, data: &[u8]) -> std::result::Result<(), super::TerminalError> {
        ...
        self.parser.parse(data, &mut self.buffer).map_err(|e| super::TerminalError::Other(e.to_string()))?;
        Ok(())
    }
    ...
    fn resize(&mut self, cols: u16, rows: u16) -> std::result::Result<(), super::TerminalError> {
        ...
        self.buffer.resize(cols, rows).map_err(|e| super::TerminalError::ResizeError(e.to_string()))?;
        Ok(())
    }
    ...
    fn set_settings(&mut self, settings: super::TerminalSettings) -> std::result::Result<(), super::TerminalError> {
...
impl Default for TerminalEmulatorManager {
    fn default() -> Self {
        Self {
...
impl TerminalEmulatorManager {
    ...
    pub fn write_to_emulator(&mut self, id: &Uuid, data: &[u8]) -> std::result::Result<(), super::TerminalError> {
    ...
    pub fn resize_emulator(&mut self, id: &Uuid, cols: u16, rows: u16) -> std::result::Result<(), super::TerminalError> {
        if let Some(emulator) = self.get_mut_emulator(id) {
