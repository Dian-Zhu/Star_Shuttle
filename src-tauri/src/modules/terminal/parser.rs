use super::buffer::{Cell, TerminalBuffer};
...
impl TerminalParser {
    ...
    fn process_osc(&mut self, byte: u8, _buffer: &mut TerminalBuffer) -> Result<()> {
        ...
        if byte == 0x07
            || (byte == 0x1b
                && self.escape_sequence.len() > 2
                && self.escape_sequence.as_bytes()[self.escape_sequence.len() - 2] == b'\\')
        {
            self.state = ParserState::Ground;
            self.escape_sequence.clear();
        } else {
    ...
    fn execute_sequence(
        ...
        match sequence {
            ControlSequence::CursorUp(n) => buffer
                .set_cursor_position(buffer.cursor_col, buffer.cursor_row.saturating_sub(n))?,
            ControlSequence::CursorDown(n) => buffer.set_cursor_position(
                buffer.cursor_col,
                buffer.cursor_row.saturating_add(n).min(buffer.rows - 1),
            )?,
            ControlSequence::CursorForward(n) => buffer.set_cursor_position(
                buffer.cursor_col.saturating_add(n).min(buffer.cols - 1),
                buffer.cursor_row,
            )?,
            ControlSequence::CursorBack(n) => buffer
                .set_cursor_position(buffer.cursor_col.saturating_sub(n), buffer.cursor_row)?,
            ControlSequence::CursorNextLine(n) => {
                for _ in 0..n {
                    buffer.set_cursor_position(
                        0,
                        buffer.cursor_row.saturating_add(1).min(buffer.rows - 1),
                    )?;
                }
            }
            ...
            ControlSequence::CursorHorizontalAbsolute(col) => {
                buffer.set_cursor_position(col.min(buffer.cols - 1), buffer.cursor_row)?
            }
    ...
    fn handle_tab(&self, buffer: &mut TerminalBuffer) {
        ...
        if let Err(e) =
            buffer.set_cursor_position(next_tab_stop.min(buffer.cols - 1), buffer.cursor_row)
        {
    ...
    fn handle_printable(&self, c: char, buffer: &mut TerminalBuffer) {
        ...
        } else if buffer.cursor_row < buffer.rows - 1 {
...
impl ControlSequence {
    fn from_csi(final_char: char, params: Vec<u16>) -> Self {
        match final_char {
            'A' => ControlSequence::CursorUp(params.first().copied().unwrap_or(1)),
            'B' => ControlSequence::CursorDown(params.first().copied().unwrap_or(1)),
            'C' => ControlSequence::CursorForward(params.first().copied().unwrap_or(1)),
            'D' => ControlSequence::CursorBack(params.first().copied().unwrap_or(1)),
            'E' => ControlSequence::CursorNextLine(params.first().copied().unwrap_or(1)),
            'F' => ControlSequence::CursorPrecedingLine(params.first().copied().unwrap_or(1)),
            'G' => ControlSequence::CursorHorizontalAbsolute(params.first().copied().unwrap_or(1)),
            'H' | 'f' => {
                let row = params.first().copied().unwrap_or(1);
                let col = params.get(1).copied().unwrap_or(1);
                ControlSequence::CursorPosition(row, col)
            }
            'J' => ControlSequence::EraseInDisplay(params.first().copied().unwrap_or(0) as u8),
            'K' => ControlSequence::EraseInLine(params.first().copied().unwrap_or(0) as u8),
            'S' => ControlSequence::ScrollUp(params.first().copied().unwrap_or(1)),
            'T' => ControlSequence::ScrollDown(params.first().copied().unwrap_or(1)),
            'm' => ControlSequence::SetGraphicsRendition(params.iter().map(|&p| p as u8).collect()),
            _ => ControlSequence::Unknown(format!(
                "CSI {} {}",
                params
                    .iter()
                    .map(|p| p.to_string())
